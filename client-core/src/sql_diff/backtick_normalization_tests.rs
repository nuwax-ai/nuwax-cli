...
fn test_index_column_with_and_without_backticks() {
    ...
    let sql_with_backticks = "CREATE TABLE test (`id` INT, `name` VARCHAR(50), INDEX idx_name (`name`));";
    let sql_without_backticks = "CREATE TABLE test (id INT, name VARCHAR(50), INDEX idx_name (name));";
    ...
    // 查找 idx_name 索引
    let index1 = table1.indexes.iter().find(|i| i.name == "idx_name").expect("应该有 idx_name 索引");
    let index2 = table2.indexes.iter().find(|i| i.name == "idx_name").expect("应该有 idx_name 索引");
...
fn test_primary_key_with_and_without_backticks() {
    // 测试主键：带反引号 vs 不带反引号
    let sql_with_backticks = "CREATE TABLE test (`id` INT, `name` VARCHAR(50), PRIMARY KEY (`id`));";
    ...
    // 查找主键
    let pk1 = table1.indexes.iter().find(|i| i.is_primary).expect("应该有主键");
    let pk2 = table2.indexes.iter().find(|i| i.is_primary).expect("应该有主键");
...
fn test_diff_with_same_structure_different_backticks() {
    ...
    let (diff_sql, _description) = generate_schema_diff(
        Some(sql_mysql),
        sql_file,
        Some("MySQL"),
        "文件"
    ...
    assert!(meaningful_lines.is_empty(), 
        "相同结构的表不应产生差异，但生成了: {:?}", meaningful_lines);
}
...
fn test_diff_with_multiple_tables_mixed_backticks() {
    ...
    assert!(meaningful_lines.is_empty(), 
        "相同结构的多个表不应产生差异，但生成了: {:?}", meaningful_lines);
}
...
fn test_composite_primary_key_with_backticks() {
    // 测试复合主键（带反引号）
    let sql_with_backticks = "CREATE TABLE test (`user_id` INT, `order_id` INT, PRIMARY KEY (`user_id`, `order_id`));";
    let sql_without_backticks = "CREATE TABLE test (user_id INT, order_id INT, PRIMARY KEY (user_id, order_id));";
...
fn test_unique_index_with_backticks() {
    ...
    let sql_with_backticks = "CREATE TABLE test (`id` INT, `email` VARCHAR(100), UNIQUE KEY `uk_email` (`email`));";
    let sql_without_backticks = "CREATE TABLE test (id INT, email VARCHAR(100), UNIQUE KEY uk_email (email));";
    ...
    let uk1 = table1.indexes.iter().find(|i| i.is_unique && !i.is_primary).expect("应该有唯一索引");
    let uk2 = table2.indexes.iter().find(|i| i.is_unique && !i.is_primary).expect("应该有唯一索引");

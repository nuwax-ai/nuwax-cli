...
pub fn generate_mysql_diff(
    ...
    for table_name in from_tables.keys() {
        if !to_tables.contains_key(table_name) {
            tracing::warn!("⚠️  检测到表 `{}` 在新版本中被删除，为确保数据安全，不生成 DROP TABLE 语句", table_name);
            diff_sql.push(format!("-- ⚠️  警告: 表 `{table_name}` 在新版本中不存在，但为了数据安全未生成删除语句"));
            diff_sql.push(format!("-- 如需删除，请手动执行: DROP TABLE IF EXISTS `{table_name}`;"));
            diff_sql.push("".to_string());
...
fn generate_column_diffs(
    ...
    for col_name in old_columns.keys() {
        if !new_columns.contains_key(col_name) {
            tracing::warn!("⚠️  检测到表 `{}` 的列 `{}` 在新版本中被删除，为确保数据安全，不生成 DROP COLUMN 语句", table_name, col_name);
            diffs.push(format!(
...
fn generate_index_diffs(
    ...
    for (idx_name, idx_def) in &old_indexes {
        ...
        // 真正需要删除的索引（仅警告）
        if idx_def.is_primary {
            tracing::warn!("⚠️  检测到表 `{}` 的主键在新版本中被删除，为确保数据安全，不生成 DROP PRIMARY KEY 语句", table_name);
            diffs.push(format!("-- ⚠️  警告: 主键在新版本中不存在，但为了数据安全未生成删除语句"));
            diffs.push(format!("-- 如需删除，请手动执行: ALTER TABLE `{table_name}` DROP PRIMARY KEY;"));
            stats.indexes_dropped += 1;
        } else {
            tracing::warn!("⚠️  检测到表 `{}` 的索引 `{}` 在新版本中被删除，为确保数据安全，不生成 DROP KEY 语句", table_name, idx_name);
            diffs.push(format!("-- ⚠️  警告: 索引 `{idx_name}` 在新版本中不存在，但为了数据安全未生成删除语句"));
            diffs.push(format!("-- 如需删除，请手动执行: ALTER TABLE `{table_name}` DROP KEY `{idx_name}`;"));
            stats.indexes_dropped += 1;
        }
    }
        ...
        if let Some(old_idx) = old_indexes.get(idx_name) {
            if old_idx != new_idx {
                ...
                if old_idx.is_primary {
                    tracing::warn!("⚠️  检测到表 `{}` 的主键定义发生变化，需要先删除旧主键，为确保数据安全，不自动生成删除语句", table_name);
                    diffs.push(format!("-- ⚠️  警告: 主键定义已变化，需要先手动删除旧主键"));
                    diffs.push(format!("-- 请手动执行: ALTER TABLE `{table_name}` DROP PRIMARY KEY;"));
                } else {
                    tracing::warn!("⚠️  检测到表 `{}` 的索引 `{}` 定义发生变化，需要先删除旧索引，为确保数据安全，不自动生成删除语句", table_name, idx_name);
                    diffs.push(format!("-- ⚠️  警告: 索引 `{idx_name}` 定义已变化，需要先手动删除旧索引"));
                    diffs.push(format!("-- 请手动执行: ALTER TABLE `{table_name}` DROP KEY `{idx_name}`;"));
                }
